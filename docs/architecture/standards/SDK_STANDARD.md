...
| 4 | **Storage** | The App's own tenant-scoped storage, through the Core-owned port | **A database handle, a connection, SQL, an ORM model, or any Cloudflare storage type.** No exception for first-party Apps (`.claude/rules/security.md` §4) | `0006` — tenancy model undecided |
...
| SD3 | **The Storage surface depends on the tenancy model** — `0006` (Proposed). | The surface's shape does not change with the model; its implementation does. Specify now, build after `0006`. |
