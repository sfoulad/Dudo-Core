...
| **CD-9** | **AS1 — the executable form and transport of a Dudo contract is undecided**, and `API_STANDARD.md` AS1 says an ADR is needed *before the first contract is written*. | This contract follows the precedent already in `packages/contracts/` — JSON Schema draft 2020-12 plus YAML — rather than selecting a new form. **Nothing here is VALIDATED:** no JSON Schema implementation exists in this repository, so these schemas are hand-checked, never machine-validated. **CORRECTED 2026-09-09 — this said "nothing here is executed" and "cross-file `urn:` `$ref`s need a resolver that does not exist", and both are now true only of validation.** `docs/decisions/0037`'s generator reads these schemas and emits TypeScript from them, resolving URN `$ref`s to do it. **So the schemas are GENERATED FROM and not VALIDATED AGAINST**, which are different capabilities that one sentence had been collapsing since before the generator existed. | Team Lead |
