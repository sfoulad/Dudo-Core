 ...
 *
 * Source:    packages/contracts/core/tenant-admin/tenant-configuration-v1.schema.json
...
import type { LevelLabels, TemplateId, TemplateName } from '../platform/template-v1.ts';

export type ErrorResponse = ErrorEnvelope;

export type AdoptedTemplate = {
  readonly template_id: TemplateId;
  readonly name: TemplateName;
  readonly level_labels: LevelLabels;
};

export type GetConfigurationOutput = {
  readonly template: AdoptedTemplate | null;
};


export type TenantOrganizationConfigurationGetError = 'unauthenticated' | 'forbidden' | 'rate_limited' | 'internal' | 'unavailable' | 'timeout';
export const TenantOrganizationConfigurationGetPermission = 'core.organization.read' as const;
