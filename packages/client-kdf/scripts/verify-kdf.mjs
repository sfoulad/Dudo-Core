 ...
 * `src/kdf.ts` directly, so this checks the code both browsers ship and not a
...
/*
 * ===========================================================================
 * TWO CHARACTERS ARE SPELLED RATHER THAN TYPED. NEITHER MAY GO BACK TO A RAW
 * BYTE, AND THE REASON IS DIFFERENT FOR EACH.
 * ===========================================================================
 *
 * Spelled 2026-09-09. **This is the only spelling of these characters in the
 * repository, and the rule is: they are spelled here or they are not written.**
 *
 * The sentence that stood here said `platform/admin/scripts/verify-kdf.mjs`
 * *"already spells them for the same reason, so this is a convention being
 * converged on rather than invented."* **That file is the predecessor this one
 * replaced** — the two host suites were unioned into this package (ADR 0040) —
 * so "converged on" named a second implementation that no longer exists. **The
 * reasoning below was right and is unchanged; only the claim of company was
 * false**, and it is the simpler and truer sentence without it.
 *
 * U+0308 COMBINING DIAERESIS — the important one.
 *   `DECOMPOSED` below is the whole point of the NFC amendment: it must be a
 *   DIFFERENT byte sequence from `COMPOSED` while rendering identically. Typed
 *   as a raw combining mark it was **unreproducible by the tools that edit this
 *   repository**: `Write` cannot emit a combining sequence at all, and a
 *   read-then-write round trip silently precomposes `o` + U+0308 into U+00F6.
 *   Any whole-file rewrite therefore collapsed the pair into one spelling.
 *
 *   THAT WOULD NOT HAVE BEEN SILENT — the guard below is written for exactly
 *   this scenario and turns two checks red. But being caught by a test after the
 *   fact is weaker than being impossible, and `architecture.md` §3a prefers the
 *   second: a hazard nobody can walk into beats a hazard something shouts about.
 *
 * U+007F DEL — the cosmetic one, and it is genuinely cosmetic.
 *   It was a raw byte inside the string literal on the `DEL (0x7F) refused`
 *   check: invisible in an editor, invisible in review, and the last finding
 *   keeping `npm run check:source-bytes` from exiting 0. **A DEL is not a NUL**
 *   and must not be treated with that protocol — it greps and diffs normally.
 *   It is fixed because it is invisible, not because it is dangerous.
 *
 * THE VALUES HAVE NOT MOVED. Both are asserted below rather than trusted, and
 * the code-point identity of COMPOSED and DECOMPOSED was compared byte for byte
 * against the previous file when they were respelled.
 */
...
const DECOMPOSED = `pa${COMBINING_DIAERESIS}sswo${COMBINING_DIAERESIS}rd`;
...
console.log('\n=== The two spelled characters are the bytes they claim to be ===\n');

/*
 * THE FLOOR FOR THE RESPELL. `String.fromCharCode` moves the risk from "a byte
 * nobody can see" to "an argument nobody checked", so the arguments are checked.
 * Without this the respell would be exactly the class of change it was made to
 * prevent — correct-looking and unverified.
 */
check('COMBINING_DIAERESIS is U+0308', COMBINING_DIAERESIS.codePointAt(0), 0x0308);
check('DEL is U+007F', DEL.codePointAt(0), 0x7f);
...
check('DEL (0x7F) refused', isSubmittableIdentifier(`sam@example.com${DEL}`), false);
 ...
 *
 * IT IS NOW BELT AND BRACES RATHER THAN THE ONLY DEFENCE, and it is kept for
 * that reason. `DECOMPOSED` is built from `String.fromCharCode(0x0308)` above,
 * so the collapse this guard catches can no longer be introduced by an editor or
 * by a tool that rewrites the file. The guard still fires if someone edits the
 * expression itself, which is the remaining way in.
 */
check(
...
/*
 * THE JS PRIMITIVE, ASSERTED SEPARATELY FROM OUR WRAPPER.
 *
 * `NFC composes the decomposed form` below goes through `normalizePassword` and
 * therefore proves what OUR function does. This proves the two strings really
 * are the same text under NFC at all — so if `normalizePassword` were ever
 * reduced to the identity, the check below would go red for the right reason
 * rather than because the premise had quietly stopped holding.
 *
 * Added 2026-09-09 from `platform/admin`'s suite as part of ADR 0040's union:
 * this is the one assertion that console had and this one lacked.
 */
...
console.log('password = UTF-8 of NFC(password). NFC, NOT NFKC — see @dudo/client-kdf src/kdf.ts.');
